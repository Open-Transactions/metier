...
#include "metier/skins/metier/widget/accountstatus.hpp"
#include "metier/skins/metier/widget/blockchainchooser.hpp"
#include "metier/skins/metier/widget/blockchainsend.hpp"
#include "metier/skins/metier/widget/licenses.hpp"
#include "metier/skins/metier/widget/receivingaddress.hpp"
...
struct ChainToolboxManager {
    ...
    auto currentChain() const noexcept
    {
        ...
        return common::convert(items_.at(position));
    }

    auto reconcile(common::Api::BlockchainList current) noexcept
    {
        auto lock = rLock{lock_};
...
private:
    ...
    auto create_tab(const int position, const int chain) noexcept -> void
    {
        ...
        auto [widget, description] = make_widget(common::convert(chain));
        toolbox_->insertItem(
        ...
        -> std::pair<std::unique_ptr<QWidget>, std::string>
    {
        ...
        auto postcondition = common::ScopeGuard{[&]() {
            layout.release();
            view.release();
            receive.release();
            send.release();
