...
{
EnterPassphrase::EnterPassphrase(QWidget *parent, const QString& displayString, bool runOneOrTwo)
    : QDialog(parent)
    ...
    , imp_(*imp_p_)
{
    ...
    connect(
        passphrase, &QLineEdit::textChanged, [this]() { imp_.passphraseChanged(); });
    connect(
        retype, &QLineEdit::textChanged, [this]() { imp_.retypeChanged(); });
}

auto EnterPassphrase::secret(opentxs::Secret& theSecret, QString& qstrSecret) const -> bool
{
