...
//-----------------------------------------------------------------------------
// 'MatterFi_SeedSizeComboBox.qml'
// Used to style drop down selection Seed Size ComboBox.
//-----------------------------------------------------------------------------
ComboBox {
  id: contextRoot
  ...
  property var requiredWordCount: 12 // place holder gets set by model selection
  ...
  // get the current word cound value
  function getWordCount() {
    var word_count = parseInt(contextRoot.displayedText)
    return word_count
  }
  ...
  delegate: ItemDelegate {
    ...
    contentItem: Text {
      ...
      Component.onCompleted: {
        console.log("SeedSize model ComboBox data:", index);
        ...
        if (QML_Debugger.hasFunction(model, "index")) {
          ...
          console.log("SeedSize model lacks an 'index' function.");
        }
      }
      */
    }
  ...
  popup: Popup {
    ...
    // close
    onClosed: {
      // update the word input fields to match the selection type
      contextRoot.requiredWordCount = contextRoot.getWordCount()
