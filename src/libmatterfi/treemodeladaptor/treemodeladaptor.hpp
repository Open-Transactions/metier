...
// https://doc.qt.io/qt-5/qlist-obsolete.html 'Build error 'QSet'
// QT_DEPRECATED_VERSION_5_15' https://doc.qt.io/qt-5/qset.html
//
...
class TreeModelAdaptor : public QAbstractListModel
{
    ...
    Q_PROPERTY(QModelIndex rootIndex READ rootIndex WRITE setRootIndex RESET
                   resetRootIndex NOTIFY rootIndexChanged)
    struct TreeItem;

public:
    explicit TreeModelAdaptor(QObject* parent = nullptr);
    ...
    QVariant data(const QModelIndex&, int role) const final;
    ...
    Q_INVOKABLE QItemSelection selectionForRowRange(
    ...
    void removeVisibleRows(
        int startIndex,
        int endIndex,
        bool doRemoveRows = true);
...
private slots:
    ...
    void modelLayoutChanged(
        const QList<QPersistentModelIndex>& parents,
        QAbstractItemModel::LayoutChangeHint hint);
    void modelRowsAboutToBeInserted(
    ...
    void modelRowsAboutToBeMoved(
        ...
        int destinationRow);
    void modelRowsAboutToBeRemoved(
        const QModelIndex& parent,
        int start,
        int end);
...
private:
    ...
    // std::unordered_set<QPersistentModelIndex> m_expandedItems;
