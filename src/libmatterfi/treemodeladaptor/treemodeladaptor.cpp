...
#define ASSERT_CONSISTENCY()                                                   \
    Q_ASSERT_X(                                                                \
        testConsistency(true /* dumpOnFail */),                                \
        Q_FUNC_INFO,                                                           \
        "Consistency test failed")
#else
#define ASSERT_CONSISTENCY qt_noop
#endif
TreeModelAdaptor::TreeModelAdaptor(QObject* parent)
...
QAbstractItemModel* TreeModelAdaptor::model() const { return m_model; }
void TreeModelAdaptor::setModel(QAbstractItemModel* arg)
{
    struct Cx {
        const char* signal;
        const char* slot;
...
void TreeModelAdaptor::clearModelData()
{
    beginResetModel();
    m_items.clear();
...
const QModelIndex& TreeModelAdaptor::rootIndex() const { return m_rootIndex; }
void TreeModelAdaptor::setRootIndex(const QModelIndex& idx)
{
    if (m_rootIndex == idx) return;
    if (m_model) clearModelData();
    m_rootIndex = idx;
    if (m_model) showModelTopLevelItems();
    emit rootIndexChanged();
}
void TreeModelAdaptor::resetRootIndex() { setRootIndex(QModelIndex()); }
QHash<int, QByteArray> TreeModelAdaptor::roleNames() const
{
    if (!m_model) return QHash<int, QByteArray>();
...
int TreeModelAdaptor::rowCount(const QModelIndex&) const
{
...
QVariant TreeModelAdaptor::data(const QModelIndex& index, int role) const
{
    ...
    switch (role) {
        ...
        case HasChildrenRole:
            return !(modelIndex.flags() & Qt::ItemNeverHasChildren) &&
                   m_model->hasChildren(modelIndex);
        case HasSiblingRole:
            return modelIndex.row() !=
                   m_model->rowCount(modelIndex.parent()) - 1;
        case ModelIndexRole:
            return modelIndex;
        default:
...
int TreeModelAdaptor::itemIndex(const QModelIndex& index) const
{
    ...
    for (int j = qMin(totalCount, m_lastItemIndex - localCount) - 1; j >= 0;
         --j) {
        const TreeItem& item = m_items.at(j);
        if (item.index == index) {
            m_lastItemIndex = j;
...
bool TreeModelAdaptor::isVisible(const QModelIndex& index)
{
    return itemIndex(index) != -1;
}
bool TreeModelAdaptor::childrenVisible(const QModelIndex& index)
{
    return (index == m_rootIndex && !m_items.isEmpty()) ||
           (m_expandedItems.contains(index) && isVisible(index));
}
const QModelIndex& TreeModelAdaptor::mapToModel(const QModelIndex& index) const
{
    return m_items.at(index.row()).index;
}
QModelIndex TreeModelAdaptor::mapRowToModelIndex(int row) const
{
    if (!m_model) return QModelIndex();
    if (row < 0 || row >= m_items.count()) return QModelIndex();
    return m_items.at(row).index;
}
QItemSelection TreeModelAdaptor::selectionForRowRange(
    const QModelIndex& fromIndex,
    const QModelIndex& toIndex) const
{
    int from = itemIndex(fromIndex);
    int to = itemIndex(toIndex);
    if (from == -1) {
        if (to == -1) return QItemSelection();
...
void TreeModelAdaptor::showModelTopLevelItems(bool doInsertRows)
{
    if (!m_model) return;
    ...
    if (topLevelRowCount == 0) return;
    showModelChildItems(
        TreeItem(m_rootIndex),
        0,
        static_cast<int>(topLevelRowCount - 1),
        doInsertRows);
}
void TreeModelAdaptor::showModelChildItems(
    ...
    bool doExpandPendingRows)
{
    ...
    int rowIdx = parentIndex.isValid() && parentIndex != m_rootIndex
    ...
    if (m_model->rowCount(parentIndex) == 0) {
        if (m_model->hasChildren(parentIndex) &&
            m_model->canFetchMore(parentIndex))
    ...
    if (start == 0) {
        ...
        // previous, as the latter is potentially buggy, see QTBUG-66062
        const QModelIndex& nextSiblingIdx =
            m_model->index(end + 1, 0, parentIndex);
        if (nextSiblingIdx.isValid()) {
            startIdx = itemIndex(nextSiblingIdx);
        } else {
            const QModelIndex& prevSiblingIdx =
                m_model->index(start - 1, 0, parentIndex);
    ...
    for (int i = 0; i < insertCount; i++) {
        ...
        if (expanded) m_itemsToExpand.append(&m_items[startIdx + i]);
    }
    if (doInsertRows) endInsertRows();
    if (doExpandPendingRows) expandPendingRows(doInsertRows);
}
void TreeModelAdaptor::expand(const QModelIndex& idx)
{
    ASSERT_CONSISTENCY();
    if (!m_model) return;
    Q_ASSERT(!idx.isValid() || idx.model() == m_model);
    if (!idx.isValid() || !m_model->hasChildren(idx)) return;
    if (m_expandedItems.contains(idx)) return;
...
void TreeModelAdaptor::collapse(const QModelIndex& idx)
{
    ASSERT_CONSISTENCY();
    if (!m_model) return;
    Q_ASSERT(!idx.isValid() || idx.model() == m_model);
...
bool TreeModelAdaptor::isExpanded(const QModelIndex& index) const
{
    ASSERT_CONSISTENCY();
    if (!m_model) return false;
...
{
    if (!m_model || isExpanded(n)) return;
    TreeItem& item = m_items[n];
    if ((item.index.flags() & Qt::ItemNeverHasChildren) ||
        !m_model->hasChildren(item.index))
        return;
    item.expanded = true;
...
void TreeModelAdaptor::collapseRow(int n)
{
    ...
    if ((item.index.flags() & Qt::ItemNeverHasChildren) ||
        !m_model->hasChildren(item.index) || childrenCount == 0)
        return;
...
int TreeModelAdaptor::lastChildIndex(const QModelIndex& index)
{
    if (!m_expandedItems.contains(index)) return itemIndex(index);
    QModelIndex parent = index.parent();
    QModelIndex nextSiblingIndex;
    while (parent.isValid()) {
        nextSiblingIndex = parent.sibling(parent.row() + 1, 0);
        if (nextSiblingIndex.isValid()) break;
        parent = parent.parent();
    }
    int firstIndex = nextSiblingIndex.isValid() ? itemIndex(nextSiblingIndex)
                                                : m_items.count();
    return firstIndex - 1;
}
void TreeModelAdaptor::removeVisibleRows(
    int startIndex,
    int endIndex,
    bool doRemoveRows)
{
    if (startIndex < 0 || endIndex < 0 || startIndex > endIndex) return;
    if (doRemoveRows) beginRemoveRows(QModelIndex(), startIndex, endIndex);
    m_items.erase(m_items.begin() + startIndex, m_items.begin() + endIndex + 1);
    if (doRemoveRows) endRemoveRows();
}
void TreeModelAdaptor::modelHasBeenDestroyed()
{
    // The model has been deleted. This should behave as if no model was set
    clearModelData();
    emit modelChanged(nullptr);
}
void TreeModelAdaptor::modelHasBeenReset()
{
    clearModelData();
    showModelTopLevelItems();
    ASSERT_CONSISTENCY();
}
void TreeModelAdaptor::modelDataChanged(
    const QModelIndex& topLeft,
    const QModelIndex& bottomRigth,
    const QVector<int>& roles)
{
    ...
    if (topIndex == -1)  // 'parent' is not visible anymore, though it's been
                         // expanded previously
        return;
    for (int i = topLeft.row(); i <= bottomRigth.row(); i++) {
        // Group items with same parent to minize the number of 'dataChanged()'
        // emits
...
void TreeModelAdaptor::modelLayoutAboutToBeChanged(
...
void TreeModelAdaptor::modelLayoutChanged(
    const QList<QPersistentModelIndex>& parents,
    QAbstractItemModel::LayoutChangeHint hint)
{
    Q_UNUSED(hint);
    ...
    for (const QPersistentModelIndex& pmi : parents) {
        if (m_expandedItems.contains(pmi)) {
            ...
            if (row != -1) {
                ...
                if (rowCount > 0) {
                    const QModelIndex& lmi =
                        m_model->index(rowCount - 1, 0, pmi);
...
void TreeModelAdaptor::modelRowsAboutToBeInserted(
...
void TreeModelAdaptor::modelRowsInserted(
    const QModelIndex& parent,
    int start,
    int end)
{
...
void TreeModelAdaptor::modelRowsAboutToBeRemoved(
    const QModelIndex& parent,
    int start,
    int end)
{
    ...
    if (parent == m_rootIndex || childrenVisible(parent)) {
        ...
        if (endIndex == -1) endIndex = itemIndex(emi);
...
void TreeModelAdaptor::modelRowsRemoved(
    const QModelIndex& parent,
    int start,
    int end)
{
...
void TreeModelAdaptor::modelRowsAboutToBeMoved(
    ...
    int destinationRow)
{
    ...
    if (!childrenVisible(destinationParent)) {
        ...
        int startIndex =
            itemIndex(m_model->index(sourceStart, 0, sourceParent));
        ...
        if (destinationRow == m_model->rowCount(destinationParent)) {
            const QModelIndex& emi2 =
                m_model->index(destinationRow - 1, 0, destinationParent);
            destIndex = lastChildIndex(emi2) + 1;
        } else {
            destIndex =
                itemIndex(m_model->index(destinationRow, 0, destinationParent));
        }
        int totalMovedCount = endIndex - startIndex + 1;
        const bool visibleRowsMoved =
            startIndex != destIndex &&
            beginMoveRows(
                QModelIndex(), startIndex, endIndex, QModelIndex(), destIndex);
        const QList<TreeItem>& buffer =
            m_items.mid(startIndex, totalMovedCount);
        int bufferCopyOffset;
        if (destIndex > endIndex) {
            for (int i = endIndex + 1; i < destIndex; i++) {
                m_items.swapItemsAt(i, i - totalMovedCount);  // Fast move from
                                                              // 1st to 2nd
                                                              // position
            }
        ...
        if (visibleRowsMoved) endMoveRows();
        if (depthDifference != 0) {
            const QModelIndex& topLeft =
                index(bufferCopyOffset, 0, QModelIndex());
...
void TreeModelAdaptor::modelRowsMoved(
    const QModelIndex& sourceParent,
    int sourceStart,
    int sourceEnd,
    const QModelIndex& destinationParent,
    int destinationRow)
{
    if (childrenVisible(destinationParent)) {
        if (!childrenVisible(sourceParent))
            ...
            int destIndex = -1;
            if (destinationRow == m_model->rowCount(destinationParent)) {
                const QModelIndex& emi =
                    m_model->index(destinationRow - 1, 0, destinationParent);
                destIndex = lastChildIndex(emi) + 1;
            } else {
                destIndex = itemIndex(
                    m_model->index(destinationRow, 0, destinationParent));
            }
            const QModelIndex& emi = m_model->index(
                destinationRow + sourceEnd - sourceStart, 0, destinationParent);
            int endIndex = -1;
            if (isExpanded(emi)) {
                int rowCount = m_model->rowCount(emi);
                if (rowCount > 0)
                    endIndex =
                        lastChildIndex(m_model->index(rowCount - 1, 0, emi));
            }
            if (endIndex == -1) endIndex = itemIndex(emi);
...
void TreeModelAdaptor::dump() const
{
    if (!m_model) return;
    int count = m_items.count();
    if (count == 0) return;
    int countWidth = static_cast<int>(floor(log10(double(count))) + 1);
    qInfo() << "Dumping" << this;
...
bool TreeModelAdaptor::testConsistency(bool dumpOnFail) const
{
    if (!m_model) {
        if (!m_items.isEmpty()) {
            qWarning()
    ...
    for (int i = 0; i < m_items.count(); i++) {
        ...
        if (item.index.parent() != parent) {
            ...
            qWarning() << "    stored index parent" << item.index.parent()
                       << "model parent" << parent;
            isConsistent = false;
        }
        ...
        if (item.expanded && !m_expandedItems.contains(item.index)) {
            ...
            qWarning() << "    set" << m_expandedItems.contains(item.index)
                       << "item" << item.expanded;
            isConsistent = false;
        }
        if (!isConsistent) {
        ...
        if (item.expanded) firstChildIndex = m_model->index(0, 0, idx);
